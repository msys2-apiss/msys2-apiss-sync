...
| **4** | mirror-merge | `yarn mirror-merge`, `mirror-merge.yml` on branch **`msys2-apiss-mirror-merge`** |
...
| Area | Detail doc |
|------|------------|
| Block 1: mirror-init | [`mirror-init.md`](mirror-init.md) |
...
and the Block 4 workflow branch to **`msys2-apiss/msys2-apiss-sync`**, then dispatches Block 3 on each pushed mirror repo.
...
|------|--------------|------------------|----------|
| `msys2-apiss-sync` (tooling repo) | Yes (TypeScript + templates) | Block 2: `mirror-poll.yml` on `main`; Block 4: `mirror-merge.yml` on branch **`msys2-apiss-mirror-merge`** (installed by Block 1) | Block 3 `workflow_dispatch_mirror_merge` (package mirrors) |
...
| **1** | `msys2-apiss/msys2-apiss-sync` (**local checkout**) | None | `yarn mirror-init` (`fetch-mirrors`) `[--push] [--repo <name>]` | From tooling repo code: initialize Block 3 on **`msys2-apiss/*`** + Block 4 CI branch on tooling; with **`--push`**: push to msys2-apiss org then dispatch Block 3 on pushed repos | Block 3/4 workflows deployed; **`--push`**: Block 3 triggered |
...
Block 2 does **not** run during Block 1 **`--push`** (Block 1 dispatches Block 3 directly).
...
| Block 1 `--push` dispatches Block 3 | Implemented |
